...
# @app.route('/api/stockCheck', methods=['GET'])
# def getStockCheck():
#    # Retrieve the 'threshold' from the query parameters
#     threshold = request.args.get('threshold', type=int)
    
#     if threshold is None:
#         return jsonify({"error": "Threshold parameter is required and must be an integer"}), 400

#     # Query stocks joined with portfolios, filtered by the threshold
#     stocks = db.session.query(Stock).join(Portfolio).filter(Portfolio.net_profit_loss >= threshold).all()
...
#             'id': stock.id,
#             'name': stock.name,
#             'portfolios': [{'id': portfolio.id, 'net_profit_loss': portfolio.net_profit_loss} for portfolio in stock.portfolios if portfolio.net_profit_loss >= threshold]
#         })
...
class PortfolioResource(Resource):
    @token_required
    def get(current_user, self):  # Switch order to ensure current_user is correctly received
        portfolios = Portfolio.query.filter_by(user_id=current_user.id).all()
        result = [
            {
                'stock_symbol': entry.stock.symbol,
